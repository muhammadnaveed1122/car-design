...
function TargetCarManage() {
  ...
  useEffect(() => {
    carService.getUniqueValues({
      ...
      dispatch(updateFilter({ referal: 'TAR' }));
    });
  }, []);

  const filterRandomCars = () => {
    let { data } = cars;
    const res = data && data.filter((data) => data.referal);
    cars = [{ data: res }];
  }
  ...
  return (
    <>
      <Helmet title="Trade Dept | Target Cars" />
...
export default isAuth(TargetCarManage) 